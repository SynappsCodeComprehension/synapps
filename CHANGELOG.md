...
- **Drag teleport fix when physics is off** — drag-end no longer nulls `fx`/`fy`; nodes stay pinned at their dragged position so the next drag-start reads the correct coordinates and cursor tracking is smooth with no snap/jump
