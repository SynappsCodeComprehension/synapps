...
- **E2E browser tests — D3 graph interactions** — `tests/e2e/test_graph_interactions.py` with `test_click_opens_detail_panel` (click opens node-detail-panel, close dismisses it), `test_dblclick_expands_node` (dispatch_event dblclick, node count stays >= initial), and `test_rightclick_removes_node` (contextmenu removes node); all SVG interactions use `force=True`
- **API integration tests** — `tests/integration/test_api.py` with session-scoped `web_client` fixture and `api_integration` pytest marker; covers all 5 FastAPI route modules (search, navigate, analysis, query, config) with valid-input and error-input tests; serialization tests verify real neo4j `Node` and `Relationship` objects pass through `serialize_result()` and produce JSON-serializable dicts
...
- **CLI golden file tests** — `tests/unit/test_cli_golden.py` with 7 syrupy snapshot tests covering `callers`, `callees`, `hierarchy`, `search`, and `doctor` commands; output normalization strips ANSI codes, replaces absolute paths with `<PATH>`, forces COLUMNS=80 for cross-machine stability
- **MCP schema unit tests** — `tests/unit/test_mcp_schema.py` with 5 tests validating tool registration metadata and naming conventions: all 21 tools have non-empty descriptions, valid `inputSchema` with `type==object`, correct tool count, no forbidden parameter names, and all parameters use snake_case
...
### Added
- **Webviewer backend routes** — added `/read_symbol`, `/assess_impact`, and `/find_implementations` HTTP endpoints to the web UI backend, matching the v3.0 MCP tool surface
- **Webviewer frontend** — updated tool sidebar, context menu, and node detail panel to match v3.0 surface: replaced `get_hierarchy` with `find_implementations`, added `assess_impact` action, replaced `scope` select with `members_only` checkbox, added `read_symbol`/`assess_impact`/`find_implementations` to sidebar, text result renderer extracts `.content` from wrapped responses
...
- **CLI `synapps context` command** — updated to use `--members-only` flag instead of removed `--scope` option
- **Onboarding template** — `agent_instructions.py` updated with v3.0 tool hierarchy (primary/secondary tools, no stale scope references)
