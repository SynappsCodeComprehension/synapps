...
- **`assess_impact` core logic** — `ContextBuilder.assess_impact()` assembles 5-section markdown report (Direct Callers ≤15, Transitive Callers ≤10 via 2-hop CALLS query, Test Coverage ≤5 with `find_tests_for`→`find_test_coverage` fallback, Interface Contract, HTTP Endpoint ≤5); all sections always present with "No X found" for empty; truncated sections show `(showing N of M)` in header; `SynappsService.assess_impact()` delegates after resolving with `preference="concrete"`
