...
def create_app(service: SynappsService, *, static_dir: Path | None = None) -> FastAPI:
    ...
    from synapps.web.routes import search, navigate, analysis, query as query_routes, config, health, tool_history
    ...
    app.include_router(health.router(service), prefix="/api")
    app.include_router(tool_history.router(service), prefix="/api")
