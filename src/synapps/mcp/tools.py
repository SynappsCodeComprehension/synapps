...
def register_tools(mcp: object, service: SynappsService, project_path: str = "") -> None:
    ...
    def get_hierarchy(full_name: str) -> str:
        """This tool has been removed. Use get_context_for instead -- it includes type hierarchy in its output."""
        return "This tool has been removed. Use get_context_for instead -- it includes type hierarchy in its output."
    ...
    def find_dependencies(full_name: str) -> str:
        """This tool has been removed. Use get_context_for instead -- it includes dependencies in its output."""
        return "This tool has been removed. Use get_context_for instead -- it includes dependencies in its output."
    ...
    def find_entry_points(full_name: str) -> str:
        """This tool has been removed. Use get_architecture instead -- it includes entry point analysis."""
        return "This tool has been removed. Use get_architecture instead -- it includes entry point analysis."
    ...
    def find_dead_code(
        ...
        exclude_file_pattern: optional regex applied to file_path to filter additional methods
          (e.g. '.*test_helpers.*' excludes files whose path contains 'test_helpers',
          '.*generated.*' excludes generated source files).
          Use alternation for multiple patterns: 'pattern1|pattern2'.
          Empty string means no additional filtering.
        ...
        subdirectory: optional path fragment to restrict results to a subdirectory
          (e.g. 'src/api'). Matches against file_path using path-segment-bounded regex
          ('api' matches '/src/api/' but not '/src/graphapi/').
    ...
    @mcp.tool()
    def find_tests_for(full_name: str) -> str:
    ...
    def find_untested(
        ...
        """
        _auto_sync_check()
        return service.find_untested(exclude_pattern=exclude_pattern, exclude_file_pattern=exclude_file_pattern, limit=limit, offset=offset, subdirectory=subdirectory)
