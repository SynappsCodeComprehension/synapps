...
def install(
    path: str = typer.Argument(default=".", help="Project path (used for Copilot detection)"),
    force: bool = typer.Option(False, "--force", help="Overwrite existing hook scripts"),
    agent: Optional[str] = typer.Option(None, "--agent", help="Target a single agent: claude, cursor, copilot"),
) -> None:
    """Install Synapps pre-tool hooks for detected AI coding agents."""
    ...
    agents = detect_agents(project_path=Path(abs_path))

    if agent:
        agents = [a for a in agents if a.name == agent]
        if not agents:
            typer.echo(f"Agent '{agent}' not detected on this machine.", err=True)
            raise typer.Exit(1)

    if not agents:
        typer.echo("No supported agents detected.")
        raise typer.Exit(0)
    ...
    installed: list[str] = []
    for a in agents:
        if typer.confirm(f"Install Synapps hooks for {a.display_name}?", default=True):
            install_scripts(hooks_dir, force=force)
            script_path = f"~/.synapps/hooks/{_SCRIPT_NAME[a.name]}"
            _UPSERT[a.name](a.config_path, script_path)
            installed.append(a.display_name)

    if installed:
        typer.echo(f"Hooks installed for: {', '.join(installed)}")
    else:
...
@app.command(rich_help_panel="Setup & Diagnostics")
def uninstall() -> None:
    """Remove Synapps pre-tool hooks from all agents."""
    from synapps.hooks.detector import detect_agents
    ...
    _REMOVE = {
        ...
        "cursor": remove_cursor_hook,
        "copilot": remove_copilot_hook,
    }

    removed: list[str] = []
    for a in agents:
        if a.config_path.exists():
            _REMOVE[a.name](a.config_path)
            removed.append(a.display_name)

    remove_scripts(hooks_dir)

    if removed:
        typer.echo(f"Hooks removed for: {', '.join(removed)}")
    else:
        typer.echo("No agent configs to clean up.")
    if not hooks_dir.exists():
        typer.echo("Hook scripts removed.")
