...
def test_get_context_for_basic():
    """Web route calls get_context_for with members_only=False and structured=True by default."""
    client, svc = _make_client()
    svc.get_context_for.return_value = {"source": "...", "callees": []}
    response = client.get("/api/get_context_for?full_name=A.B")
    assert response.status_code == 200
    svc.get_context_for.assert_called_once_with("A.B", members_only=False, max_lines=-1, structured=True)


def test_get_context_for_with_members_only():
    """Web route passes members_only query param to service."""
    client, svc = _make_client()
    svc.get_context_for.return_value = {"members": ["do_work"]}
    response = client.get("/api/get_context_for?full_name=A.B&members_only=true")
    assert response.status_code == 200
    svc.get_context_for.assert_called_once_with("A.B", members_only=True, max_lines=-1, structured=True)
