...
def test_sync_project_uses_plugin_detection(tmp_path):
    ...
    with patch("synapps.service.indexing._sync_project", return_value=fake_result) as mock_sync:
        with patch("synapps.service.indexing.Indexer") as mock_indexer_cls:
...
def test_sync_project_uses_predetected_files(tmp_path):
    ...
    with patch("synapps.service.indexing._sync_project", return_value=fake_result) as mock_sync:
        with patch("synapps.service.indexing.Indexer"):
            svc = SynappsService(conn, registry=registry)
