...
def test_check_staleness_stale_file() -> None:
    ...
    with patch("synapps.graph.lookups.os.path.getmtime", return_value=datetime(2026, 3, 11, 11, 0, tzinfo=timezone.utc).timestamp()):
        with patch("synapps.graph.lookups.os.path.exists", return_value=True):
...
def test_check_staleness_fresh_file() -> None:
    ...
    conn = _conn([[indexed_at, "/proj/Foo.cs"]])
    with patch("synapps.graph.lookups.os.path.getmtime", return_value=datetime(2026, 3, 11, 10, 0, tzinfo=timezone.utc).timestamp()):
        with patch("synapps.graph.lookups.os.path.exists", return_value=True):
            result = check_staleness(conn, "/proj/Foo.cs")
    assert result is not None
...
def test_check_staleness_file_deleted_from_disk() -> None:
    ...
    with patch("synapps.graph.lookups.os.path.exists", return_value=False):
