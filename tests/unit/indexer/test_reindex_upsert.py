...
@patch("synapps.indexer.indexer.get_file_symbol_names", return_value={"Ns.Foo", "Ns.Foo.DoWork", "Ns.Foo.OldMethod"})
def test_reindex_does_not_call_delete_file_nodes(mock_get_syms, mock_del_orphans, mock_del_edges, tmp_path):
    ...
    with patch("synapps.indexer.indexer.delete_file_nodes") as mock_dfn, \
         patch("synapps.indexer.indexer.SymbolResolver") as MockResolver:
...
@patch("synapps.indexer.indexer.delete_outgoing_edges_for_file")
@patch("synapps.indexer.indexer.delete_orphaned_symbols")
@patch("synapps.indexer.indexer.get_file_symbol_names", return_value={"Ns.Foo"})
def test_reindex_does_not_call_collect_or_restore_summaries(mock_get_syms, mock_del_orphans, mock_del_edges, tmp_path):
    ...
    with patch("synapps.indexer.indexer.collect_summaries") as mock_cs, \
         patch("synapps.indexer.indexer.restore_summaries") as mock_rs, \
         patch("synapps.indexer.indexer.SymbolResolver") as MockResolver:
        MockResolver.return_value = MagicMock()
        indexer.reindex_file(str(cs_file), str(tmp_path))
...
@patch("synapps.indexer.indexer.delete_outgoing_edges_for_file")
@patch("synapps.indexer.indexer.delete_orphaned_symbols")
@patch("synapps.indexer.indexer.get_file_symbol_names", return_value={"Ns.Foo", "Ns.Foo.OldMethod"})
